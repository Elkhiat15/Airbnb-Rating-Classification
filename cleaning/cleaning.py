...
def handle_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "host_response_rate" in df.columns:
        df["host_response_rate"] = pd.to_numeric(
            df["host_response_rate"].astype(str).str.replace("%", ""), errors="coerce"
        )
    ...
    if "room_type" in df.columns and df["room_type"].isnull().any():
        ...
        df["room_type"] = df["room_type"].fillna(
            df["room_type"].mode()[0] if not df["room_type"].mode().empty else "Unknown"
        )
    ...
    for col in numeric_impute_cols:
        if col in df.columns and df[col].isnull().any():
            ...
            if "room_type" in df.columns and df["room_type"].notna().all():
                ...
                df[col] = df[col].fillna(df[col].median())
            logger.info(
                f"  Imputed {missing_count} missing values in {col} with median"
            )
    ...
    if "host_response_rate" in df.columns:
        if df["host_response_rate"].isnull().any():
            ...
            logger.info(
                f"  Imputed {missing_count} missing host_response_rate with median; added has_response_rate flag"
            )
    ...
    if "review_scores_rating" in df.columns:
        ...
        if dropped > 0:
            logger.info(
                f"  Dropped {dropped} rows with missing target (review_scores_rating)"
            )
    ...
    logger.info(
        f"  Missing value handling: {initial_rows} : {len(df)} rows ({initial_rows - len(df)} dropped)"
    )
    return df
...
def handle_outliers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "price" in df.columns:
        ...
        logger.info(
            f"  Capped price: [{before_min:.2f}, {before_max:.2f}] → [{after_min:.2f}, {after_max:.2f}]"
        )
    ...
    for col in numeric_cols:
        if col in df.columns:
            ...
            if IQR < 0.01:
                logger.info(
                    f"  Skipped IQR capping for {col} (IQR={IQR:.4f} too small)"
                )
                continue
            ...
            if outliers_count > 0:
                ...
                logger.info(
                    f"  Capped {outliers_count} outliers in {col} using IQR method (bounds: [{lower_bound:.2f}, {upper_bound:.2f}])"
                )
