...
from modelling.class_balancing import (
...
class TestErrorAnalysis:
    ...
    def test_with_feature_data(self):
        ...
        result = error_analysis(
            y_true, y_pred, X_test=X, feature_names=["f1", "f2", "f3"]
        )
...
class TestPrintSummary:
    def test_runs_without_error(self, rating_labels, capsys):
        y_true, y_pred = rating_labels
        metrics = {
            **standard_metrics(y_true, y_pred),
            **business_metrics(y_true, y_pred),
        }
    ...
    def test_single_configs_have_single_values(self):
        ...
        for name, cfg in SINGLE_CONFIGS.items():
            for param, values in cfg["params"].items():
                assert len(values) == 1, (
                    f"{name}.{param} has {len(values)} values, expected 1"
                )
...
class TestPrepareData:
    def test_prepare_data_shapes(self, train_test_dfs):
        ...
        X_train, X_test, y_train, y_test, preprocessor, le = prepare_data(
            train_df, test_df
        )
