...
class TestNormalizeColumns:
    ...
    def test_normalize_handles_missing_columns(self):
        ...
        df = pd.DataFrame({"city": ["NYC", "LA"], "other_col": [1, 2]})
        result = normalize_columns(df)
...
class TestHandleOutliers:
    ...
    def test_price_capped_at_domain_bounds(self):
        ...
        df = pd.DataFrame({"price": [5, 50, 500, 5000, 15000]})
    ...
    def test_iqr_capping_for_numeric_columns(self):
        ...
        assert result["number_of_reviews"].max() <= result[
            "number_of_reviews"
        ].quantile(0.75) + 1.5 * (
            result["number_of_reviews"].quantile(0.75)
            - result["number_of_reviews"].quantile(0.25)
        )

    def test_handle_outliers_preserves_valid_data(self):
        """Test that valid data points are not modified."""
        df = pd.DataFrame(
            {"price": [100, 200, 300, 400, 500], "accommodates": [2, 3, 4, 5, 6]}
        )
        result = handle_outliers(df)
    ...
    def test_handle_outliers_with_missing_columns(self):
        ...
        df = pd.DataFrame({"price": [100, 200, 300], "other_col": [1, 2, 3]})
        result = handle_outliers(df)
...
class TestCleaningPipeline:
    ...
    def test_pipeline_order_matters(self):
        ...
        assert list(df["city"]) == [
            "New York",
            "San Francisco",
        ]  # LA row dropped due to missing price
    ...
    def test_clean_pipeline_end_to_end(self, tmp_path):
        ...
        assert result["host_identity_verified"].dtype == bool
        assert result["host_response_rate"].dtype in [
            np.float64,
            np.int64,
        ]  # Can be int or float
