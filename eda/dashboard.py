...
def load_data(data_path: str = "data/processed/cleaned.csv"):
    ...
    try:
        ...
        existing_categories = [
            cat for cat in category_order if cat in df["rating_category"].unique()
        ]
        df["rating_category"] = pd.Categorical(
            df["rating_category"], categories=existing_categories, ordered=True
        )
...
def build_prediction_features(
    ...
    room_type_std = (
        room_type_std if room_type_std and not np.isnan(room_type_std) else 1.0
    )
...
def show_eda_section(df):
    ...
    st.markdown(
        '<p class="sub-header">Exploratory Data Analysis</p>', unsafe_allow_html=True
    )
    ...
    with col4:
        ...
        st.metric(
            "High Rated", f"{high_rated_pct:.1f}%", help="Listings with 4.51+ rating"
        )
    with col5:
    ...
    with col1:
        show_binned = st.checkbox(
            "Show binned categories", value=True, key="target_binned"
        )
    ...
    fig = plot_target_distribution(
        df, target_col="review_scores_rating", binned_col=binned_col, figsize=(10, 4)
    )
    st.pyplot(fig)
    plt.close()
    ...
    with col1:
        y_axis_options = [
            col
            for col in ["review_scores_rating", "price", "number_of_reviews"]
    ...
    with col3:
        plot_type = st.selectbox(
            "Plot Type:", ["box", "violin"], key="feature_plot_type"
        )
    ...
    with col1:
        method = st.selectbox(
            "Correlation Method:", ["pearson", "spearman", "kendall"], key="corr_method"
        )
    with col2:
    ...
    fig = plot_correlation_heatmap(
        df, features=features_to_plot, method=method, annot=show_annot, figsize=(9, 7)
    )
    st.pyplot(fig)
    plt.close()
    ...
    color_option = st.selectbox(
        "Color by:", ["rating_category", "price"], key="geo_color"
    )
...
def show_predict_section(featured_df):
    ...
    st.markdown(
        '<p class="sub-header">Rating Prediction Demo</p>', unsafe_allow_html=True
    )
    ...
    with st.form("prediction_form"):
        ...
        with col1:
            ...
            neighbourhood_options = sorted(
                featured_df.loc[featured_df["city"] == city, "neighbourhood"]
                .dropna()
                .astype(str)
                .unique()
                .tolist()
            ...
            property_type = st.selectbox(
                "Property Type:", ["Apartment", "House", "Condo", "Townhouse", "Loft"]
            )
        ...
        with col2:
            ...
            bathrooms = st.number_input(
                "Bathrooms:", min_value=0, max_value=10, value=1
            )
        ...
        with col3:
            ...
            price = st.number_input(
                "Price per Night ($):", min_value=10, max_value=2800, value=100
            )
            amenity_count = st.number_input(
                "Number of Amenities:", min_value=0, max_value=100, value=20
            )

            st.markdown("**Reviews**")
            number_of_reviews = st.number_input(
                "Total Reviews:", min_value=0, max_value=500, value=10
            )
    ...
    if submitted:
        try:
            ...
            predicted_category = label_map.get(
                predicted_category_raw, str(predicted_category_raw)
            )
...
def main():
    ...
    # Header
    st.markdown(
        '<p class="main-header">Airbnb Rating Classification Dashboard</p>',
        unsafe_allow_html=True,
    )
    ...
    if df is None:
        st.error(
            "Failed to load data. Please check that data files exist in data/processed/"
        )
        st.stop()
