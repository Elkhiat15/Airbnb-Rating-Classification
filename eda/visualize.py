...
def plot_correlation_heatmap(
    ...
    ax.set_title(
        f"Feature Correlation Heatmap ({method.capitalize()})", fontsize=11, pad=12
    )
    ax.tick_params(axis="both", labelsize=8)
...
def plot_feature_vs_target(
    ...
    if categorical_target and categorical_target in df.columns:
        ...
        axes[0].set_title(
            f"{feature_col.replace('_', ' ').title()} vs Rating (Continuous)"
        )
        axes[0].tick_params(axis="x", rotation=45)
        axes[0].grid(alpha=0.3, axis="y")
        ...
        cross_tab = (
            pd.crosstab(df[feature_col], df[categorical_target], normalize="index")
            * 100
        )
        cross_tab.plot(
        ...
        axes[1].legend(
            title="Rating Category", bbox_to_anchor=(1.05, 1), loc="upper left"
        )
        axes[1].grid(alpha=0.3, axis="y")
    else:
        fig, ax = plt.subplots(1, 1, figsize=(10, 5))
...
def plot_geospatial_scatter(
    ...
    for idx, city in enumerate(cities):
        ...
        if is_categorical:
            ...
            cbar = plt.colorbar(
                scatter, ax=axes[idx], label=color_by.replace("_", " ").title()
            )
            cbar.ax.tick_params(labelsize=7)
    ...
    fig.suptitle(
        f"Geospatial Distribution Colored by {color_by.replace('_', ' ').title()}",
        fontsize=11,
        y=1.00,
    )
    plt.tight_layout()
...
def plot_missing_values(
    df: pd.DataFrame, figsize: Tuple[int, int] = (12, 8)
) -> plt.Figure:
    ...
    if cols_with_missing:
        sns.heatmap(
            df_sample[cols_with_missing].isnull(),
            cbar=False,
            yticklabels=False,
            cmap="coolwarm",
            ax=axes[1],
...
def plot_numeric_distributions(
    ...
    for idx, col in enumerate(numeric_cols):
        ...
        # Histogram with KDE
        axes[idx].hist(
            data, bins=30, edgecolor="black", alpha=0.6, color="skyblue", density=True
        )
...
def plot_price_by_room_type(
    ...
    sns.violinplot(
        data=df_plot, x=room_type_col, y="log_price", ax=axes[0], palette="Set2"
    )
...
def plot_amenity_analysis(
    ...
    if amenity_count_col not in df.columns:
        ...
        if "amenities" in df.columns:
            ...
            df_plot[amenity_count_col] = df_plot["amenities"].apply(
                lambda x: (
                    len(eval(x)) if isinstance(x, str) and x.startswith("[") else 0
                )
            )
        else:
    ...
    axes[0].axvline(
        df_plot[amenity_count_col].mean(),
        color="red",
        linestyle="--",
        linewidth=2,
        label=f"Mean: {df_plot[amenity_count_col].mean():.1f}",
    )
    axes[0].axvline(
        df_plot[amenity_count_col].median(),
        color="green",
        linestyle="--",
        linewidth=2,
        label=f"Median: {df_plot[amenity_count_col].median():.1f}",
    )
    ...
    ax2.set_xticklabels(
        [
            ...
            for interval in amenity_rating.index
        ],
        rotation=45,
        ha="right",
    )
