...
def run_pipeline(
    ...
    logger.info(
        f"  Featured dataset: {df_featured.shape[0]} rows × {df_featured.shape[1]} columns"
    )
    ...
    logger.info(
        f"  Ready dataset: {df_ready.shape[0]} rows × {df_ready.shape[1]} columns"
    )
    ...
    analysis_feature_cols = [
        ...
        if col not in ["review_scores_rating", "rating_category"]
    ]
    ...
    if dropped_for_modeling:
        logger.info(
            f"Features in analysis but not modeling: {sorted(dropped_for_modeling)}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", default="data/processed/cleaned.csv", help="Path to cleaned CSV file"
    )
    parser.add_argument(
        "--output", default="data/processed", help="Directory to save output files"
    )
    parser.add_argument(
        "--train-ratio",
        type=float,
