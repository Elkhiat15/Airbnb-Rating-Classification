...
def add_price_ratios(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "price" in df.columns and "beds" in df.columns:
        ...
        logger.info(
            f"  Created price_per_bed feature (mean: ${df['price_per_bed'].mean():.2f})"
        )
...
def add_price_relative_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "price" in df.columns and "room_type" in df.columns:
        ...
        df["price_relative_to_room_type"] = df.apply(
            lambda row: (
                (row["price"] - room_type_stats.loc[row["room_type"], "median"])
                / (
                    room_type_stats.loc[row["room_type"], "std"]
                    if room_type_stats.loc[row["room_type"], "std"] > 0
                    else 1
                )
            ),
            axis=1,
    ...
    if "price" in df.columns and "city" in df.columns:
        ...
        df["price_relative_to_city"] = df.apply(
            lambda row: (
                (row["price"] - city_stats.loc[row["city"], "median"])
                / (
                    city_stats.loc[row["city"], "std"]
                    if city_stats.loc[row["city"], "std"] > 0
                    else 1
                )
            ),
            axis=1,
