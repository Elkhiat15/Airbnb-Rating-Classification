...
def correlation_filter(df: pd.DataFrame, threshold: float = 0.95) -> list[str]:
    ...
    logger.info(
        f"  Correlation filter: removed {len(to_drop)} highly correlated features (threshold={threshold})"
    )
    if to_drop:
...
def select_features(
    ...
    logger.info(
        f"Feature selection complete: {len(X.columns)} -> {len(features_to_keep)} features"
    )
...
def drop_unwanted_features(
    df: pd.DataFrame, features_to_drop: list = None
) -> pd.DataFrame:
    ...
    if features_to_drop:
        ...
        logger.info(
            f"  Dropped {len(features_to_drop)} unwanted features: {features_to_drop}"
        )
    else:
        logger.info(
...
def create_train_test_split(
    ...
    if not existing_stratify_cols:
        logger.warning(
            f"None of the stratify columns {stratify_cols} found in DataFrame. Using target only."
        )
        stratify_key = df[target_col]
    else:
        stratify_key = df[existing_stratify_cols].apply(
    ...
    logger.info(
        f"  Train split: {len(train_df)} rows ({len(train_df) / len(df) * 100:.1f}%)"
    )
    logger.info(
        f"  Test split:  {len(test_df)} rows ({len(test_df) / len(df) * 100:.1f}%)"
    )
    ...
    for split_name, split_df in [("Train", train_df), ("Test", test_df)]:
        ...
        logger.info(
            f"    {split_name} - {target_col} distribution: {target_dist.to_dict()}"
        )
        if "room_type" in split_df.columns:
            room_dist = split_df["room_type"].value_counts(normalize=True).sort_index()
            logger.info(
                f"    {split_name} - room_type distribution: {room_dist.to_dict()}"
            )
