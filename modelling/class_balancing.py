...
def apply_borderline_smote(X_train, y_train, strategy="auto"):
    ...
    bsmote = BorderlineSMOTE(sampling_strategy=strategy, random_state=42)
...
def compute_sample_weights(y_train, strategy="balanced"):
    ...
    if strategy == "balanced":
        ...
        class_weights = {
            cls: total / (n_classes * count) for cls, count in counts.items()
        }

    elif strategy == "sqrt":
        # Square root of inverse frequency (less aggressive)
        class_weights = {cls: np.sqrt(total / count) for cls, count in counts.items()}
