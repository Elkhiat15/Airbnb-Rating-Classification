...
class DataValidator:
    ...
    def check_accuracy(self) -> dict:
        ...
        def set_check(col, valid_set, mostly=1.0):
            ...
            threshold_pct = (1 - mostly) * 100
            passed = violation_pct <= threshold_pct
            invalid_vals = list(normalised[~normalised.isin(valid_lower)].unique()[:10])
            checks.append(
                {
    ...
    def check_completeness(self) -> dict:
        ...
        logger.info(
            f"  Completeness: {passed_count}/{len(checks)} column checks passed"
        )
        return report
    ...
    def check_consistency(self) -> dict:
        ...
        if {"latitude", "longitude"} <= set(self.df.columns):
            us_mask = self.df["latitude"].between(self.LAT_MIN, self.LAT_MAX) & self.df[
                "longitude"
            ].between(self.LON_MIN, self.LON_MAX)
            add(
    ...
    def check_uniqueness(self) -> dict:
        ...
        if {"latitude", "longitude"} <= set(self.df.columns):
            coord_dup = int(self.df.duplicated(subset=["latitude", "longitude"]).sum())
            checks.append(
                {
    ...
    def check_outliers(self) -> dict:
        ...
        report = {
            ...
            "methods_used": [
                "IQR (Tukey fences, multiplier=1.5)",
    ...
    def check_relationships(self) -> dict:
        ...
        for col_a, col_b in expected_positive_pairs:
            ...
            spearman_r, spearman_p = stats.spearmanr(
                numeric_df[col_a], numeric_df[col_b]
            )
            passed = pearson_r > 0
            pair_results.append(
                {
        ...
        if target in numeric_df.columns:
            feature_cols = [
                c for c in self.NUMERIC_COLS if c in numeric_df.columns and c != target
            ]
    ...
    def run_all(self) -> dict:
        ...
        dim_statuses = {
            k: v.get("overall_status", "N/A") for k, v in self.results.items()
        }
