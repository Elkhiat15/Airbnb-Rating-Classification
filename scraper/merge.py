...
def merge_sources(
    ...
    scraped_df["id"] = (
        scraped_df["listing_url"].str.extract(r"/rooms/(\d+)")[0].astype(np.int64)
    )

    # Drop unnecessary columns
    logger.info("   Dropping unnecessary columns from scraped data")
    scraped_df = scraped_df.drop(
        columns=["listing_url", "is_superhost", "free_cancellation"]
    )

    # Rename columns to match Kaggle
    logger.info("   Renaming columns to match Kaggle schema")
    scraped_df = scraped_df.rename(
        columns={
            "guests": "accommodates",
    ...
    logger.info(
        f"   Scraped data: {scraped_df.shape[0]:,} rows × {scraped_df.shape[1]} cols"
    )
    logger.info(
        f"   Kaggle data:  {kaggle_df.shape[0]:,} rows × {kaggle_df.shape[1]} cols"
    )
    ...
    logger.info(
        f"Merged Dataset:     {merged_df.shape[0]:,} rows × {merged_df.shape[1]} cols"
    )
